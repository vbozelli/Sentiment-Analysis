...
def analiseSentimento(resposta):
	...
	if sentimentoAlchemyApi != None:
		...
		return (feature, sentimentoAlchemyApi)
	return None
pool4 = Pool()
comeco = datetime.now()
featuresClassificados = pool4.map(analiseSentimento, classificados)
tempo = datetime.now() - comeco
pool4.terminate()
pool4.close()
while None in featuresClassificados:
	featuresClassificados.remove(None)
precisao = accuracy(classificador, featuresClassificados) * 100
iteracao = iteracao + 1
arquivoMedicoes = open('medicoes_analise_processos' + str(iteracao) + '.txt', 'w')
...
if precisao > 50:
	featuresTemp = []
	for feature, sentimento in featuresClassificados:
		if classificador.classify(feature) == sentimento:
			featuresTemp.append((feature, sentimento))
	features.extend(featuresTemp)
	shuffle(features)
	...
	arquivoNeutros.close()
arquivoResultados = open('resultados_sem_stopwords_' + str(iteracao) + '.csv', 'w', newline='')
